...
export const LearningModeSwitch: React.FC<LearningModeSwitchProps> = ({ value, onChange, size = 'md', className = '' }) => {
  const options: Array<{ mode: LearningMode; label: string; icon: React.ReactNode; title: string }> = [
    { mode: 'learn', label: 'Learn', icon: <BookOpen size={size === 'sm' ? 12 : 14} />, title: 'Learn & Understand: theory, examples and a try-it before each new idea' },
    { mode: 'practice', label: 'Practice', icon: <Zap size={size === 'sm' ? 12 : 14} />, title: 'Practice mode: straight into the challenges' }
  ];
  return (
