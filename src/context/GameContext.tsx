...
import { applyProgress, contentService, stagesForTrack, stageStatus } from '../services/contentService';
...
import { DEFAULT_LANGUAGE, LANGUAGE_TRACKS, LanguageTrack } from '../data/tracks';
...
export interface LanguageTrackProgress {
  track: LanguageTrack;
  /** This track's stages only, in track order, with per-track lock/progress state. */
  stages: Stage[];
  cleared: number;
  total: number;
...
export interface GameContextType {
  /* content — the WHOLE bank, every language, unfiltered. Used by marketing
     pages and platform-wide totals (Achievements, Landing). */
  stages: Stage[];
  allChallenges: Challenge[];
  challengeById: (id: string) => Challenge | undefined;

  /* content — scoped to the learner's currently selected language track.
     This is "your path": Learn, the Challenge library and the practice
     session's auto-target-picking all read these instead of the raw,
     cross-language `stages`/`allChallenges` above. */
  selectedLanguage: SupportedLanguage;
  setSelectedLanguage: (language: SupportedLanguage) => void;
  languageTracks: LanguageTrackProgress[];
  learnerStages: Stage[];
  ...
  /** Server-verified: is a remote compiler (Judge0) configured for languages
   *  beyond JavaScript/Python? Never carries the credentials themselves. */
  judge0Configured: boolean;

  /* actions */
  completeChallenge: (challenge: Challenge, options?: SolveOptions) => Promise<number>;
  /** Record that a Concept's teaching sequence has been shown, so it is not repeated. */
...
/** A saved language preference is only valid if it still names a real track. */
function hydrateLanguage(raw: string | null): SupportedLanguage {
  return raw && LANGUAGE_TRACKS.some((t) => t.id === raw) ? (raw as SupportedLanguage) : DEFAULT_LANGUAGE;
}
...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  /* ----------------------------------------------------- language track */
  /**
   * Which language track the learner is currently practising. Purely a
   * client-side viewing preference (like `theme`) - it decides which slice
   * of the one shared `Stage`/`Challenge` bank is shown as "your path", not
   * a second progress system. Progress itself is still just
   * `stats.completedChallenges` / `stats.completedStages`, the exact same
   * fields as before; a challenge's stage naturally belongs to one language,
   * so per-language progress falls out of filtering, not a new store.
   */
  const [selectedLanguage, setSelectedLanguageState] = useState<SupportedLanguage>(() =>
    hydrateLanguage(readString(STORAGE_KEYS.language))
  );

  useEffect(() => {
    writeString(STORAGE_KEYS.language, selectedLanguage);
  }, [selectedLanguage]);

  const setSelectedLanguage = useCallback((language: SupportedLanguage) => {
    setSelectedLanguageState(hydrateLanguage(language));
  }, []);
  ...
  const languageTracks = useMemo<LanguageTrackProgress[]>(
    () =>
      LANGUAGE_TRACKS.filter((track) => !bundle.hiddenLanguages.includes(track.id)).map((track) => {
        const trackStages = applyProgress(stagesForTrack(bundle.stages, track), stats);
        const cleared = trackStages.filter((s) => s.state === 'Completed').length;
        let totalChallenges = 0;
        let solvedChallenges = 0;
        for (const s of trackStages) {
          totalChallenges += s.challenges.length + (s.test ? 1 : 0);
          solvedChallenges += s.challenges.filter((c) => stats.completedChallenges.includes(c.id)).length;
          if (s.test && stats.completedChallenges.includes(s.test.id)) solvedChallenges += 1;
        }
        return { track, stages: trackStages, cleared, total: trackStages.length, totalChallenges, solvedChallenges };
      }),
    [bundle.stages, bundle.hiddenLanguages, stats]
  );

  const activeTrackProgress = useMemo(
    () =>
      languageTracks.find((t) => t.track.id === selectedLanguage) ??
      languageTracks[0] ?? {
        track: LANGUAGE_TRACKS[0],
        stages: [] as Stage[],
        cleared: 0,
        total: 0,
        totalChallenges: 0,
    ...
    [languageTracks, selectedLanguage]
  );

  const learnerStages = activeTrackProgress.stages;
  const learnerChallenges = useMemo(() => {
    const ids = new Set(learnerStages.map((s) => s.id));
    return bundle.challenges.filter((c) => ids.has(c.stageId));
  }, [bundle.challenges, learnerStages]);
  ...
  // The active stage may belong to any track - it is looked up by id across
  // the whole bank, not just the currently selected language, so a challenge
  // opened from a cross-language surface (e.g. the global Achievements list)
  // still resolves correctly even if it is not the learner's current track.
  ...
  const openPractice = useCallback(
    (stageId?: string, challengeId?: string) => {
      const target =
        (stageId && learnerStages.find((s) => s.id === stageId)) ||
    ...
    [learnerStages, stats.isPremium, stats.completedChallenges, notify]
  );
  ...
  const openStageTest = useCallback(
    (stageId: string) => {
      const target = learnerStages.find((s) => s.id === stageId) ?? stages.find((s) => s.id === stageId);
    ...
    [learnerStages, stages, stats, notify]
  );
   ...
   * seen this explanation before") - it does not earn XP and is never sent to
   * the server, so it stays out of the server-reconciliation path above.
   */
  const markConceptSeen = useCallback((conceptId: string) => {
    setStats((prev) =>
      prev.seenConcepts.includes(conceptId)
        ? prev
        : { ...prev, seenConcepts: [...prev.seenConcepts, conceptId] }
    );
  }, []);
