...
import { LanguageTrack } from '../data/tracks';
...
export interface ContentBundle {
  ...
  source: 'api' | 'bundle';
  /**
   * Language ids an admin has unpublished (see /admin/languages). Only ever
   * populated when content came from the API - the offline bundle has no way
   * to know about a server-side admin decision, so it shows every track,
...
export const contentService = {
  ...
  async loadFromApi(): Promise<ContentBundle | null> {
    try {
      const { stages, challenges, hiddenLanguages } = await api.content();
      ...
      return {
        ...
        source: 'api',
        hiddenLanguages: Array.isArray(hiddenLanguages) ? hiddenLanguages : []
      };
    } catch {
      return null;
    }
 ...
 *
 * This runs once over ALL stages (the whole cross-language bank, for
 * marketing/global views) and again, separately, over each language track's
 * own slice (see `stagesForTrack` below) - the locking chain is always local
 * to whatever list you pass in, so a track's first stage is never held
 * hostage by an unrelated track's last one.
 */
...
/**
 * The subset of `stages` that make up one language track, in that track's
 * order. Feeding this (rather than the whole bank) into `applyProgress` is
 * what gives each language its own independent lock chain - see the
 * `LanguageTrack` docs in `src/data/tracks.ts` for why this is a filter over
 * shared data rather than a second content system.
 */
export function stagesForTrack(stages: Stage[], track: LanguageTrack): Stage[] {
