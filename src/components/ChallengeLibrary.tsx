...
/**
 * Search and filter within the current language track.
 *
 * The staged path is the guided route; this is for the person who wants to
 * drill one specific thing ("show me every debug challenge I have not done").
 * Scoped to `learnerChallenges`/`learnerStages` (the selected language's own
 * slice) rather than the whole cross-language bank, so switching the
 * language on the Learn page changes what this library searches too.
 */
export const ChallengeLibrary: React.FC = () => {
  const { learnerChallenges: allChallenges, learnerStages: stages, stats, openPractice, openStageTest, openSubModal } = useGame();
