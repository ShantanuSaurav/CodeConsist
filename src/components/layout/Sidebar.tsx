...
export const Sidebar: React.FC<SidebarProps> = ({ onNavigate }) => {
  const { theme, toggleTheme, user, stats, logout, openAuthModal, serverStatus, languageTracks, selectedLanguage, setSelectedLanguage } =
    useGame();
  ...
  return (
    <div className="w-64 h-full border-r border-black/5 dark:border-white/5 bg-white dark:bg-[#0d1117] flex flex-col">
      <div className="p-6 flex-1 overflow-y-auto">
        <Link to="/" className="text-2xl font-bold flex items-center gap-2 mb-5" onClick={onNavigate}>
          <span className="text-[var(--color-primary)] font-mono">&lt;/&gt;</span>
          <span className="text-gray-900 dark:text-white">Devlingo</span>
        </Link>

        {/* Persistent language switcher - always visible, always changeable,
            not just on the Learn page's language-selection screen. */}
        <label className="block mb-6">
          ...
          >
            {languageTracks.map(({ track, cleared, total }) => (
