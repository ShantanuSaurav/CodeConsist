...
import { CodeBlock, useBodyScrollLock, useFocusTrap } from '@/ui';
...
export const PracticeModal: React.FC<PracticeModalProps> = ({ readingSlot }) => {
  const { stages, completeChallenge, executeCode, stats } = useSession();
  const { activeStage, activeMode, activeChallenges, activeChallengeIndex, closePractice, goToChallenge, openStageTest, openPractice } =
    usePracticeSession();
