 ...
 * together; `stage-tests.ts` holds the mandatory tests; `stages.ts` and
 * `tracks.ts` are metadata, not content. Adding a topic is adding a folder -
 * nothing here changes.
 */
import type { Challenge } from '@/types';
import type { ContentRecord, ContentSpec } from '@/platform/content-registry';
import { STAGE_META } from './stages';
import { LANGUAGE_TRACKS } from './tracks';
...
export const challengesSpec: ContentSpec<Challenge> = {
  ...
  extensions: ['.ts'],
  exclude: ['index.ts', 'spec.ts', 'stages.ts', 'tracks.ts'],
  shape: 'array',
...
/** Re-exported so the Node loader can read stage and track metadata without touching the browser-only index. */
