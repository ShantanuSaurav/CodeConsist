...
export const SocialProof: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    return [
      { value: String(lessons), label: 'Lessons', hint: 'hand-checked' },
      { value: String(tests), label: 'Stage tests', hint: `across ${stages.length} stages` },
      { value: String(executable), label: 'Code problems', hint: 'graded by real test runs' },
      { value: String(languages), label: 'Languages', hint: 'and notations' }
    ];
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-2 md:grid-cols-4 gap-y-6 md:divide-x md:divide-border-subtle">
        {stats.map((stat, i) => (
          <div key={stat.label} className={i > 0 ? 'md:pl-8' : ''}>
            <Stat label={stat.label} value={<span className="font-mono text-2xl">{stat.value}</span>} hint={stat.hint} />
          </div>
        ))}
      </div>
    </section>
